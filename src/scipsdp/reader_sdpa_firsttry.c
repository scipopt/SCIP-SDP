...
/**
*Usage:  char LINE_BUFFER[80];
    int max_length=6;
    scanf("%[0-9a-zA-Z-. ]",LINE_BUFFER);
    double *val=(double *) malloc(80 * sizeof(double));
 	readLine(LINE_BUFFER, val,max_length);
    int i;
    for (i = 0; i < 10; i += 1){
...
)
{
    ...
    while( token != NULL ) {
        //printf( "%s\n", token );
        //printf("%s\n",token);
        *(values+i)=strtod(token, &end);//atoi(token);
        //printf("%lf\n",*(values+i));
        token = strtok(NULL, s);
        i=i+1;
        if(i>=number_of_vars){
...
)
{
    ...
    if ( sscanf(CBF_LINE_BUFFER, "%i", &(data->nvars)) != 1 )  //Tim:read only one variable instead of two
    {
    ...
    SCIP_CALL( SCIPallocBlockMemoryArray(scip, &(data->createdvars), data->nvars) );
        ...
        for (v = 0; v < data->nvars; v++)
        {
            ...
            SCIP_CALL( SCIPcreateVar(scip, &var, varname, lb, ub, 0.0, SCIP_VARTYPE_CONTINUOUS,     //Tim:immer ohne beschränkung 
   ...
   )
{
   ...
   if ( sscanf(CBF_LINE_BUFFER, "%i", &(data->nsdpaconstblock)) != 1 ) 
   {
...
)
{
   ...
   SCIP_CALL( SCIPallocBufferArray(scip, &blockVals, CBF_MAX_LINE) );//TIM:Die Hälfte würde wohl auch reichen.
   SCIP_CALL( SCIPallocBufferArray(scip, &blockValsLp, CBF_MAX_LINE) );
   SCIP_CALL( SCIPallocBufferArray(scip, &blockValsPsd, CBF_MAX_LINE) );
    ...
    nblocks= readLineInt(CBF_LINE_BUFFER, blockVals,data->nsdpaconstblock); //FRAGE: checken ob zurückgegebene Menge gleich der erwarteten Blöcke
	...
	SCIP_Real lhs;
      ...
      lhs = -SCIPinfinity(scip);    //TODO: rausfinden ob das wirklich der default von sdpa ist
      rhs = SCIPinfinity(scip);   	//TIM: Constraint wird geschrieben als lhs<=Summe(a[i]*x[i])<=rhs, erwartet wird Summe(..)>=0

      //strcmp(CBF_NAME_BUFFER, "L+") == 0 ) //FRAGE: >=?
...
{  /*lint --e{818}*/
    ...
    SCIP_CALL( SCIPallocBufferArray(scip, &objVals, CBF_MAX_LINE) );//TIM:Die Hälfte würde wohl auch reichen.
	...
	SCIPerrorMessage("until readline\n" );
    readLineDouble(CBF_LINE_BUFFER, objVals,data->nvars);
	SCIPerrorMessage("after readline\n" );


    for (v = 0; v < data->nvars; v++)              //TODO:Var startet ab null wegen Speicher, unten in HCoord die Abfrage angepasst
    {
...
)
{
  ...
  //  int* constnnonz;
    ...
    if ( data->createdvars == NULL )  //FRAGE: warum checkt Acoord auf createdvars und der rest auf nvars? =>createdvars seid ObjectAcoord (?)
    {
   ...
   // if ( nnonz < 0 )
    ...
    //constnnonz=data->sdpblocksizes; //TODO: constnnonz abzählen, bei Cbf stand es grob in der Datei)
    ...
    /* initialize sdpconstnblocknonz with 0 */
        ...
        data->sdpconstnblocknonz[b] = 0;
         ...
         //if ( data->sdpblocksizes[b]>0){//constnnonz[b] > 0 ) { //TIM:hab ich in die Schleife verschoben //Tim: keine Ahnung wo das her kommt, im cbf gibt es das nicht
    ...
    while( CBFfgets(scipfile, &linecount) == SCIP_OKAY )
    {
      //  SCIPerrorMessage("Hier sollte er gefailt sein.");
        ...
        if (sscanf(CBF_LINE_BUFFER, "%i %i %i %i %lf", &v, &b, &row, &col, &val) != 5) { //TIM: passt so
            SCIPerrorMessage("Could not read entry of HCOORD in line %"
            SCIP_LONGINT_FORMAT
            ".\n", *linecount);
            SCIPABORT();
        ...
        b=b-1;
            ...
            if (v >= 0) {  //TIM: checke ob Hcoord oder Dcoord
                ...
                if (b < 0 || b  >= ncbfsdpblocks) { 
                    SCIPerrorMessage("Given SDP-coefficient in line %" //TODO:Abfragen sind doppelt
                    SCIP_LONGINT_FORMAT
                ...
                if (SCIPisZero(scip, val)) {
                    ...
                    /* make sure matrix is in lower triangular form *///TIM: sollte nicht schaden
                    if (col > row) {
                ...
                if (b < 0 || b >= data->nsdpblocks) {
                    SCIPerrorMessage("Given constant entry in line %" //TODO: Abfragen doppelt gemoppelt
                    SCIP_LONGINT_FORMAT
                    " for SDP-constraint %d which does not exist!\n", *linecount, b);
                    SCIPABORT();
                    return SCIP_READERROR; /*lint !e527*/
            ...
            if(v>=0){ //TIM: ob ACoord oder BCoord
                ...
                //FRAGE: Im prinzip könnte man mehrmals die gleiche variable befüllen
