...
{
   SdpCone*              sdpcone;            /**< SDP cone to store SDP constraint data */
};
...
SCIP_RETCODE Blas_DGEMV(
   int                   nrows,              /**< numer of rows in matrix */
   int                   ncols,              /**< numver of cols in matrix */
...
SCIP_RETCODE computeIthEigenvalue(
   ...
   bool                  geteigenvectors,    /**< Should also the eigenvectors be computed? */
...
/** presolve-routine that adds some constraints for approximation of the sdpcone, if there is an entry on the right hand
 * side there must be a corresponding diagonal entry
 *
...
SCIP_DECL_CONSCHECK(consCheckSdp)
{
   ...
   {
      SCIP_CALL( getSdpCone(scip, conss[i], &sdpcone) );
      SCIP_CALL( consCheckSdp(scip, sdpcone, sol, checkintegrality, checklprows, printreason, result) );
      if (*result == SCIP_INFEASIBLE)
      {
         return SCIP_OKAY;
      }
...
SCIP_DECL_CONSENFOPS(consEnfopsSdp)
{
   ...
   {
      SCIP_CALL( getSdpCone(scip, conss[i], &sdpcone) );
      SCIP_CALL( consCheckSdp(scip, sdpcone, NULL, 0, 0, 0, result) );
