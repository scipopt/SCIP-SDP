...
SCIP_RETCODE SCIPsdpiSolverLoadAndSolveWithPenalty(
   ...
   SCIP_Real*            startZrow,          /**< dual matrix Z = sum Ai yi as starting point for the solver: row indices; may be NULL if starty = NULL */
   SCIP_Real*            startZcol,          /**< dual matrix Z = sum Ai yi as starting point for the solver: column indices; may be NULL if starty = NULL */
   SCIP_Real*            startZval,          /**< dual matrix Z = sum Ai yi as starting point for the solver: values; may be NULL if starty = NULL */
   SCIP_Real*            startXnblocknonz,   /**< primal matrix X as starting point for the solver: number of nonzeros for each block,
                                               *  also length of corresponding row/col/val-arrays; may be NULL if starty = NULL */
   SCIP_Real*            startXrow,          /**< primal matrix X as starting point for the solver: row indices; may be NULL if starty = NULL */
   SCIP_Real*            startXcol,          /**< primal matrix X as starting point for the solver: column indices; may be NULL if starty = NULL */
