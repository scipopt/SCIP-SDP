	 ...
	 bestnfails++;
      ...
      else if( fail )
      {
         ...
         mark = " ";
	       ...
	       ((ismini && ismaxi) ||
		(ismini && maxdb - minpb > 1e-4 * max(max(abs(maxdb), abs(minpb)), 1.0)) ||
		(ismaxi && maxpb - mindb > 1e-4 * max(max(abs(maxpb), abs(mindb)), 1.0)) ||
		(!ismini && !ismaxi && abs(maxpb - minpb) > 1e-4 * max(abs(maxpb), 1.0))) )
      {
          ...
          (!onlyfail || fail) )
      {
         ...
         if( texcmpfile != "" )
         {
            ...
            for( o = 0; o < nsolver; ++o )
            {
               ...
               if( (time[s,pidx] <= 0.5*reftime && !hitlimit[s,pidx]) || (!hitlimit[s,pidx] && refhitlimit) )
                  timecolor = "red";
               else if( (time[s,pidx] >= 2.0*reftime && !refhitlimit) || (hitlimit[s,pidx] && !refhitlimit) )
                  timecolor = "blue";
               else
                  timecolor = "black";
