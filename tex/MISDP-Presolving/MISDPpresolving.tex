...
\begin{question}
  ...
  a diagonal entry becomes 0, are the other entries in the column and row
  fixed to 0 as well?
\end{question}
...
\begin{lemma}
  ...
  \[
    -\sqrt{U_{ii}\, U_{jj}} \leq X_{ij} \leq \sqrt{U_{ij}\, U_{jj}}
  \]
...
\[
  \tilde{U}_{ij} \define \sum_{k \in [m]: A^k_{ij} > 0} A^k_{ij}\, u_i -
  \sum_{k \in [m]: A^k_{ij} < 0} A^k_{ij}\, \ell_i - A^0_{ij}.
\]
...
$(i,j)$ is contained in exactly on matrix, i.e., there exists
...
\begin{proof}
  ...
  \[
    0 \preceq V^{-\top} (\mu\, A^k - A^0) V^{-1} = \mu\,
    V^{-\top} A^k V^{-1} - V^{-\top} A^0 V^{-1} = \mu\, I - V^{-\top} A^0 V^{-1}.
  \]
  The eigenvalues of $\mu\, I - B$ for some matrix $B$ are $\mu - \lambda$
  for eigenvalues $\mu$ of $B$.\footnote{This follows, for example, by the
...
\begin{lemma}
  ...
  \]
  where $\alpha = \min\,\{y_{\ell},\mu_{\ell}\}$.
\end{lemma}
...
\begin{proof}
  ...
  First assume that
  $\alpha\, A^{\ell} + \sum_{k=1,k \neq \ell}^m \mu_k\, A^k\, y_k - A^0
  \succeq 0$.  Then for every $x \in \R_+^n$
  \begin{align*}
...
\]
where $A^+$ is the pseudo-inverse of $C$. Reversing the roles of $A$ and
$C$ we get:
\[
  D \succeq 0 \quad\Leftrightarrow\quad
...
optimal solution rank-1 solution~$X^*$. Thus, $X^* = x^* (x^*)\T$ for some
...
Nevertheless if the matrix $A\T A$ is \emph{positive definite}, we get
