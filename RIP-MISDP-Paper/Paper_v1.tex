...
establishing for $\ell_0$-$\ell_1$-equivalence. However, in~\cite{PT14} it
was shown that computing the restricted isometry constants for a given
matrix~$A$ and~$k$ is NP-hard.

One the other hand, d'Aspremont et al.~\cite{Asp07,Asp08} proposed several
...
\{x \in \R^n : \Norm{x}{0} \leq k \}$ we have
 \begin{equation}\label{rip}
  (1-\delta_k)\Norm{x}{2}^2 \leq \Norm{Ax}{2}^2 \leq (1+\delta_k) \Norm{x}{2}^2.
 \end{equation}
 \end{rmk}
...
generalized to the case where $\Norm{Ax - b}{2} \leq \epsilon$. The restricted isometry property can also be formulated in an asymmetric way like in \cite{FL09}:
...
\begin{equation}\label{asymRIP}
   \alpha_k^2 \Norm{x}{2}^2 \leq \Norm{Ax}{2}^2 \leq \beta_k^2 \Norm{x}{2}^2
\end{equation}
...
$\delta_k = \max \{ 1 - \alpha_k^2, \beta_k^2 - 1 \}$. So $A$ satisfies definition \ref{asymRIPdef} if and only if it satisfies definition \ref{ripdef}, but as in (\ref{rip}) usually only one of the two inequalities will 
...
isometry constant $\delta_k = \max \{ 1 - \alpha_k^2, \beta_k^2 - 1 \}$.
