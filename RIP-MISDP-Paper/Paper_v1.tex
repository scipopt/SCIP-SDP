...
\comment{$\max \{1 - \alpha_k^2,\; \beta_k^2 - 1\}$ erf\"ullt RIP nicht nur,
sondern ist auch das kleinste $\delta$, das es tut, weshalb die asym. wirklich
st\"arker ist.} So $A$ has a small
...
\begin{align}\label{Rk1MISDP}
 ...
 & z \in \{0,1\}^n. \nonumber
\end{align}
\comment{test whether to use only diagonal inequalities $-z_j \leq X_{jj} \leq z_j$, or only those for $j$ (which are redundant in the SDP because of diagonal dominance) or all for $i$ and $j$ (which are even more redundant 
because of symmetrie)}
...
Now let $\bar{X}$ be the $\ell \times \ell$ matrix with all rows and columns outside of $T$ removed. Then $\bar{X}$ is feasible for (\ref{Proj}) with the same objective value as the optimal solution $\hat{X}$ of (\ref{MISDP}), 
...
and $X^{\prime}_{ij}=0$ otherwise. This solution will then be feasible for (\ref{MISDP}), as $\Tr(\check{X}) = 1$ and $\check{X}\in S_\ell$ imply $0 \leq \check{X}_{jj} = X^{\prime}_{i_j i_j} \leq 1$ and therefore 
...
\section{Solving MISDPs}
\label{sec:solving_misdps}

For solving the mixed-integer semidefinite programs we used the MISDP-Solver SCIP\_SDP originally developed in \cite{MS12} and \cite{Mar13}, which combines the branch-and-bound-framework of SCIP 3.1 \cite{SCIP} with 
interior-point SDP-solvers. An important property for semidefinite programs is the slater condition, that states that there is a strictly feasible point, in this case a positive definite matrix, for the primal and dual problem, 
which is needed for strict duality \cite{alizadeh95, helmberghabil} and therefore an imposed precondition for most SDP-solvers. This will however usually be violated in a branch-and-bound-context, when variables get fixed and the
variable bounds are added as linear constraints, which will then be transformed to diagonal entries of the semidefinite program. \comment{will man das ausf\"uhrlicher erkl\"aren oder eine Quelle daf\"ur zitieren (wenn man eine 
findet) ? } To keep the slater condition intact, the code will remove all fixed variables from the SDP formulation, as well as removing zero-rows from the SDP-matrices, which might appear after fixing variables to zero. The 
SCIP\_SDP-code also extracts some easy LP-conditions from the SDP-constraints, to use the presolving capabilities of SCIP for linear constraints. 

The semidefinite problems appearing as relaxations are then solved using interior-point-methods, for details on 
those see for example the books by Ye and Nesterov and Nemirovski \cite{yeip, neneip}. In this case the dual-scaling interior-point-solver DSDP 5.8 \cite{DSDP} was used. 
Branching was always conducted on the binary variable with value closest to one, as a node-selection rule best first search was used, as it needs the smallest number of iteration to find the optimal solution \cite{achterbergphd}, 
and unlike linear solvers warmstart-capabilities cannot be used to profit from the small changes of the problems in the breadth-first-search.

%The code was overhauled and ported to C in contrast to the cited original version. 
