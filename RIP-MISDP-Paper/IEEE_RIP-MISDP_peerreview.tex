...
\begin{definition}\label{ripdef}
  ...
  \begin{equation}\label{rip}
    ...
    \quad\text{for all }x \in \Sigma_k \define \{x \in \R^n \suchthat \Norm{x}{0} \leq k\}.
  \end{equation}
\end{definition}
...
constants for a given matrix~$A$ and~$k$ is NP-hard, see~\cite{BDDS13,PT14}. 
...
D'Aspremont et al.~\cite{Asp08,Asp07} proposed several
...
hard to solve, even for matrices of smaller size. However, they are
still solvable for larger instances than a brute-force approach computing
the eigenvalues of all submatrices of corresponding size. 

Even if the techniques for solving MISDPs are not yet capable of solving
large enough instances to be used for computing RICs in practical applications,
by computing exact restricted isometry constants, our approach allows to evaluate
relaxations like those proposed by d'Aspremont et al. in \cite{Asp08,Asp07}.
In fact, these bounds turn out
to be quite tight or weak, depending on the instance, while in parts requiring
solution times comparable to those of the MISDP-approach. This complements the
...
\begin{equation}\label{Rk1MISDP}
 ...
 \end{aligned}
\end{equation}
We then relax the non-convex rank constraint to arrive at the following, still non-convex,
MISDP formulation:
...
\begin{proof}
  ...
  \dots, i_\ell\}$ be the support of $z^\star$, and define $\bar{A} =
  (A_{i_1}, A_{i_2}, \dots, A_{i_\ell})$ to be the submatrix of~$A$ formed by
  the columns indexed by~$T$. Then consider
  \begin{equation}\label{Proj}
   \begin{aligned}
    \min \quad & \Tr(\bar{A}\T \bar{A} \bar{X}), \\
    \text{s.t.} \quad & \Tr(\bar{X}) = 1, \\
    & \bar{X} \succeq 0,\; \bar{X} \in S_\ell.
   \end{aligned}
  \end{equation}
  ...
  \ell$. Therefore, $\hat{X}_{ij} \leq \sqrt{\hat{X}_{ii}\hat{X}_{jj}} \leq \hat{z}_j$ by 
  nonnegativity of the minors of positive semidefinite matrices.
  ...
  Let $\check{X}$ be an extreme point of \eqref{Proj} that attains the
  optimal objective value (since the set of
...
\newpage
\appendix{\input{tables/matrixList}}
