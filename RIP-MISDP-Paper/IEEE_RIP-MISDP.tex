...
could be solved within the time- and memory-limit. Note that in this case, we ommitted the rank one matrices, since these satisfy
...
computed via mixed-integer semidefinite programming. Although the used
matrices are necessarily small, we can significantly improve the bounds
computed by the semidefinite relaxations proposed in \cite{Asp08} and
\cite{Asp07}. Moreover, the results of this paper can be used to design
further approximations of the RIP, which might also be applicable for
larger matrices. On the negative side, the running time of the mixed-integer
semidefinite approach is significantly larger than the relaxation
of~\cite{Asp07}. On the positive side, it is faster and/or uses less memory
than the relaxation of \cite{Asp08} and it computes optimal constants!

For the future, the hope is that with the advancement of solving techniques
for mixed-integer semidefinite programs, the sizes of the matrices can be
