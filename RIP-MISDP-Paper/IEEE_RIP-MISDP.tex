...
$\Norm{x}{2}^2 = 1$ by the technique of semidefinite lifting, first introduced by Goemans and and Williamson~\cite{GW95} 
...
to make $\Sigma$ positive semidefinite again. If $\Psi$ is the
corresponding optimal objective, this implies a lower bound on $\alpha_k$
in\comment{Ist $M\,I$ statt $M$ korrekt? \\ $\Rightarrow$ nein, sollten alles Skalare sein, $\Psi$ demenstrechend optimaler Zielfunktionswert statt L\"osung (habe ich jetzt oben angepasst)}
\begin{equation*}
  \alpha_k^2 \geq \sup_{\rho \geq 0}\; (M - \Psi) - \rho\, k.
\end{equation*}
In \cite{AspBG14} explicit bounds on the gap between \eqref{Asp08} and the penalized problem \eqref{phi} were derived using randomization arguments.
...
Problem~\eqref{Asp08Dual} most of the larger instance were aborted because of the memory limit of 8GB. These are
counted with the maximum allowed time of four hours. 

In Table~\ref{lhsRhsGap} we compare the relative gap between the bounds on $\alpha_k$ and $\beta_k$ computed by the two heuristics
...
solution, with only slightly worse results for binary and rank one matrices. The formulation in~\eqref{Asp08Dual}
...
Table~\ref{lhsRhsTime}. The relaxation proposed in \cite{Asp07}
...
$\alpha_k$ and 30\,\% for $\beta_k$. If successfully solved, the relaxation
of \cite{Asp08} provides better bounds for $\beta_k$ for most types of 
