...
$N(0, 1/m)$-distributed entries, with entries chosen uniformly in $\pm 1/\sqrt{m}$ and finaly with distribution
\begin{equation}\nonumber
...
to randomly generated nine matrices of three different sizes with rows ranging from 15 to 30, columns 
...
too well. Especially the relaxation given in \cite{Asp08} regularly returned worse solution than the
...
perform much better. Problem~\eqref{Asp08Dual} constantly produced bounds within 10 to 35\,\% of the optimal
solution. The formulation in~\eqref{Asp08Dual} showed bigger fluctuation, but was very tight in many cases,
...
was very fast, taking only a few seconds for all instances. The mixed-integer semidefinite program obviously
cannot compete with these solving times, although for band-matrices, specifically, the difference was relatively
small. The Lagrangian Relaxation from \cite{Asp08}, however, took more time to produce bounds than was needed
by our MISDP to compute exact solutions. The big difference is also caused by the instances running into the 
memory limit, but even without those, the algorithm still took slightly longer than the MISDP.
...
We have seen that exact restricted isometry constants can be computed not only via total enumeration but also by solving a mixed integer semidefinite program. Because of this exactness result, the new problem could drastically
