...
For the asmmetric version of the RIP in Definition~\ref{asymRIPdef}, one
...
results hold asymptotically and with respect to high probabilty.
...
programm and show that it produces exact restricted isometry
...
\begin{align}\label{Asp08}
 ...
 \text{s.t.} \quad & \Tr(X) = 1, \tag{A2-Primal}\\
 & X \succeq 0,  \nonumber \\
 & X \succeq P_i \succeq 0 \quad \text{for } i = 1, \dots, n, \nonumber
\end{align}
for $B_i = b_i\,b_i\T - \rho\, I$, where $b_i$ is the $i$-th column of the
squareroot of $\Sigma$. For details, we refer the reader to~\cite{Asp08}. The corresponding dual problem can then be written as 
\begin{align}\label{Asp08Dual}
  \min \quad & \lambda_{\max}\Big(\sum_{i=1}^n Y_i \Big) \nonumber \\
  \text{s.t.} \quad & Y_i \succeq B_i \quad \text{for } i = 1, \dots, n \tag{A2-Dual} \\
  & Y_i \succeq 0 \quad \ \ \text{for } i = 1, \dots, n, \nonumber
\end{align}
which has only half as many variables as \eqref{Asp08} and one fewer linear and semidefinite constraint. Then \eqref{Asp08Dual} can be solved for non-negative $\rho$, which should be smaller than $\Sigma_{11}$, because otherwise the
...
\end{equation*}
\comment{$\bar{X}$ L\"osung von \eqref{Asp08}?}
In this case problem \eqref{Asp08} or equivalently \eqref{Asp08Dual} has to
be solved for $\Sigma = M I - A\T A$, where $M \in \R$ has to be big enough
to make $\Sigma$ positive semidefinite again. If $\bar{X}$ is a
corresponding optimal solution, this implies a lower bound on $\alpha_k$
in\comment{Ist $M\,I$ statt $M$ korrekt?}
\begin{equation*}
  \alpha_k^2 \geq \sup_{\rho \geq 0}\; (M\, I - \bar{X}) - \rho\, k.
\end{equation*}
In \cite{AspBG14} explicit bounds on the quality of this relaxation where derived using randomization arguments.
\comment{Ist hier die Lagrange-Relaxierung gemeint?}
