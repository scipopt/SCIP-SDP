...
\setlength{\abovecaptionskip}{1ex plus0.5ex minus0.5ex}
\setlength{\belowcaptionskip}{1ex plus0.5ex minus0.5ex}
...
\title[Computing Restricted Isometry Constants via MISDP]{Computing Restricted Isometry Constants via \\ Mixed-Integer Semidefinite Programming}
\author[T. Gally]{Tristan Gally}
\author[M. E. Pfetsch]{Marc E. Pfetsch}

\address{Tristan Gally \\ Technische Universit\"at Darmstadt, Department of Mathematics\\ Dolivostra\ss e 15, 64293 Darmstadt, Germany}
\email{gally@mathematik.tu-darmstadt.de}
\address{Marc E. Pfetsch \\ Technische Universit\"at Darmstadt, Department of Mathematics\\ Dolivostra\ss e 15, 64293 Darmstadt, Germany}
...
Compressed Sensing, Restricted Isometry Property, Mixed-Integer Semidefinite Programming
...
\begin{definition}\label{ricdef}
  ...
  \quad\text{ for all }x \in \Sigma_k \big\},
  \]
...
\begin{theorem}[Cand\'es~\cite{Can08}]\label{RIPsqrt2}
  Let the RIC of order $2k$ of $A$ satisfy the condition $\delta_{2k} < \sqrt{2}-1 \approx 0.4142$. If there
...
$\lambda Ax = \lambda b$ with $\lambda \in \R \setminus \{0\}$. In Definition~\ref{asymRIPdef}, however, we can
...
\begin{equation}\label{QP}\tag{QP}
 ...
 \end{aligned}
\end{equation}
The corresponding maximization problem allows to compute the constant $\beta_k^2$ in~\eqref{upperRIP}. D'Aspremont et al. \cite{Asp07} tackled the non-convex quadratic equality constraint 
...
\begin{proof}
  ...
  \eqref{MISDP} is bounded. Moreover, $X_{11} = 1$, $X_{ij} = 0$
  ...
  \dots, i_\ell\}$ be the support of $z^\star$, and $\check{A} =
  (A_{i_1}, A_{i_2}, \dots, A_{i_\ell})$ the submatrix of~$A$ formed by
  ...
  agree, and, furthermore, $\Rk(\hat{X}) = \Rk(\check{X})$.  Therefore, it
...
For efficient application of branch-and-bound to \eqref{MISDP}, it is
...
For solving the continuous SDPs, the command-line version of
SDPA~7.3.8 is used. We compute the left- and right-hand restricted isometry
...
funding, since parts of this research have been carried out within the
...
\input{tables/matrixList_optimization-online}
