...
\item \textsf{generateRIPA.m}: This script generates random matrices~$A$
  ...
  \end{dense_itemize}
  Output: Instances~\textsf{Matrices/type.m.n.k\_MISDP.side.pd.rank.socp.strgbnds.trineq.boundver}, where
  \begin{dense_itemize}
  \item ``type'', $m$, $n$, $k$ : as above
  \item ``side''          : ``$\ell$'' (left side/$\alpha_k$), ``r'' (right side/$\beta_k$)
  ...
  \item ``socp'' : variant of the valid inequalities from~\cite{LiX20},
    see~\eqref{eq:ValidSOCPineqSDP}. Only for right side!
    \begin{dense_itemize}
      \item 0: no valid inequalities
      \item 1: valid inequalities as SDP-constraints,
        see~\eqref{eq:ValidSOCPineqSDP}
      \item 2: valid inequalities directly as SOC-constraints
    \end{dense_itemize}
  \item ``strgbnds''      : 1 (with stronger lower bounds~$X_{ij} \geq 0$
    if~$A \geq 0$ entrywise), 0 (without)! Only for right side!
  \item ``trineq''        : 1 (add the trace constraint as inequality
    constraint, that is $\tr(X) \leq 1$ for right side, resp. $\tr(X) \geq
    1$ for th left side), 0 (use $\tr(X) == 1$ for both sides)
  \item  ``boundver''      :
    \begin{dense_itemize}
    \item 0: $-z_i \leq X_{ii} \leq z_i,\; i \in [n]$, 
    \item 1: $-z_j \leq X_{ij} \leq z_j,\; i,j \in [n]$,
    \item 2: as 1 plus strengthened off-diagonal bounds
      $-0.5z_j \leq X_{ij} \leq 0.5z_j$ for $i \neq j$.
    \end{dense_itemize}
  \end{dense_itemize}
...
\item \textsf{RIPCBFdual.m}: This script generates the MISDP formulation
  ...
  \item $A$: matrix 
  \item $k$: order
  \item ``side'', ``Rank'', ``socp'', ``strgbnds'', ``trineq'', ``boundver'': see Output
